    ...
    public function knownForMovies()
    {
        ...
        return collect($castMovies)->sortByDesc('popularity')->take(5)->map(function ( $movie ) {
            ...
            return collect($movie)->merge([
                'poster_path' => $movie['poster_path'] ? 'https://image.tmdb.org/t/p/w185' . $movie['poster_path'] : 'http://via.placeholder.com/185x278',
                'title' => $title,
                'page_link' => $movie['media_type'] === 'movie' ? route('movies.show', $movie['id']) : route('tv.show', $movie['id'])
            ]);
        });
    }
